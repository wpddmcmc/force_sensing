clear all
cam1 = webcam(2);  %%% Figure1 You have to see 25 elements by cam1, if you cannot, please change number 3 to 1 or 2 
cam2 = webcam(1);  %%% FIgure2 You have to see 10 elements by cam2, if you cannot, please change number 1 to 2 or 3 
%cam1.Resolution = '1280x720';%%new
cam2.Resolution = '1280x720';
%%
y_img = snapshot(cam1);   % five 3-axis force sensors image
img = snapshot(cam2);       % two 3axis force sensors image
figure(1)
imshow(y_img);
figure(2)
imshow(img);
...
elseif ( i>20) %%%% main force measurment. 
...
%-----------------------------Data summary-------------------
arTimeStamp(i) = cputime - timeZero;%time in seconds since matlab was started

%each force element
Force_xyz(i,:) = [fx_s1 fy_s1 fz_s1 fx_s2 fy_s2 fz_s2 fx_s3 fy_s3 fz_s3 fx_s4 fy_s4 fz_s4 fx_s5 fy_s5 fz_s5 fx_s6 fy_s6 fz_s6 fx_s7 fy_s7 fz_s7];

%force magnitude of each sensor
f_res_s1 = sqrt(fx_s1^2 + fy_s1^2 + fz_s1^2);
f_res_s2 = sqrt(fx_s2^2 + fy_s2^2 + fz_s2^2);
f_res_s3 = sqrt(fx_s3^2 + fy_s3^2 + fz_s3^2);
f_res_s4 = sqrt(fx_s4^2 + fy_s4^2 + fz_s4^2)
f_res_s5 = sqrt(fx_s5^2 + fy_s5^2 + fz_s5^2);
f_res_s6 = sqrt(fx_s6^2 + fy_s6^2 + fz_s6^2);
f_res_s7 = sqrt(fx_s7^2 + fy_s7^2 + fz_s7^2);

Force_res(i,:) = [f_res_s1, f_res_s2, f_res_s3, f_res_s4, f_res_s5, f_res_s6, f_res_s7];

%plot(sqrt(Force(i,1)^2+Force(i,2)^2+Force(i,3)^2)
end
i=i+1;
end

%%% Force is the force form the phantom.. 
%%% in the Force there are 21 force data from 7 3-axis force sensor.. 
%%%
